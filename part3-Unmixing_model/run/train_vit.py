...
from datasets.sampler import GroupSampler, FastRandomIdentitySampler, ClassAwareSampler
...
loss_names = ['FocalSymmetricLovaszHardLogLoss','DBLoss','HardL1MseDBLoss']
# transform_ = train_gan_augment
# ganmodel = ganmodel_()

parser = argparse.ArgumentParser(description='PyTorch Protein Classification')
parser.add_argument('--out_dir', type=str,default='ViTL_base_ep50_Bat32',
...
parser.add_argument('--scheduler', default='Adam45_1', type=str, help='scheduler name')
parser.add_argument('--sampler', default='ClassAware', type=str, help='name of the latest checkpoint (default: None)')

parser.add_argument('--epochs', default=50, type=int, help='number of total epochs to run (default: 55)')
parser.add_argument('--batch_size', default=32, type=int, help='train mini-batch size (default: 32)')
parser.add_argument('--workers', default=5, type=int, help='number of data loading workers (default: 3)')
parser.add_argument('--imgs_per_gpu', default=32, type=str, help='name of the latest checkpoint (default: None)')

parser.add_argument('--clipnorm', default=1, type=int, help='clip grad norm')
parser.add_argument('--freeze_layers', default=False, type=str, help='whether need to freeze layers before MLP')
...
def main():
    ...
    train_dataset = data_reader(MODE,Images_Norm_params,is_Train=True, is_Syn=False,transform=train_transform)
    ...
    valid_dataset = data_reader(MODE, Images_Norm_params, is_Train=False, is_Syn=False)
    ...
    validsyn_dataset = data_reader(MODE, Images_Norm_params, is_Train=False, is_Syn=True)
    ...
    log.write('\n')
    log.write(
        'epoch    iter      rate     |  train_loss/acc/corr/mse  |   valid_loss/acc/corr/mse   |   validsyn_loss/acc/corr/mse   |  best_epoch/best_acc  |  min \n')
    log.write('---------------------------------------------------------------------------------------------------------------------------------------------------------------------\n')
    start_epoch += 1
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        lr = lr_list[0]
        iter, train_loss, train_acc, train_corr, train_mse = train(train_loader, model, criterion, optimizer, epoch,
                                                                clipnorm=args.clipnorm, lr=lr)

        with torch.no_grad():
            valid_loss, valid_acc, val_corr, val_mse = validate(valid_loader, model, criterion, epoch, focal_loss)
            validsyn_loss, validsyn_acc, valsyn_corr, valsyn_mse = validate_syn(validsyn_loader, model, criterion,
                                                                                epoch, focal_loss)
        is_best = validsyn_acc > best_acc
        best_epoch = epoch if is_best else best_epoch
        best_acc = validsyn_acc if is_best else best_acc

        print('\r', end='', flush=True)
        log.write(
            '%5.1f   %5d    %0.6f   |  %0.4f  %0.4f  %0.4f  %0.4f  |    %0.4f  %6.4f %0.4f  %0.4f  |    %0.4f  %6.4f %0.4f  %0.4f  |  %6.1f    %6.4f   | %3.1f min \n' % \
            (
            epoch, iter + 1, lr, train_loss, train_acc, train_corr, train_mse, valid_loss, valid_acc, val_corr, val_mse,
            validsyn_loss, validsyn_acc, valsyn_corr, valsyn_mse, best_epoch, best_acc, (time.time() - end) / 60))
        ...
        save_model(model, is_best, model_out_dir, optimizer=optimizer, epoch=epoch, best_epoch=best_epoch, best_acc=best_acc)
    ...
    for iter, iter_data in enumerate(train_loader, 0):
        ...
        if (iter + 1) % print_freq == 0 or iter == 0 or (iter + 1) == num_its:
            print('\r%5.1f   %5d    %0.6f   |  %0.4f  %0.4f  %0.4f  %0.4f  | ... ' % \
                  (epoch - 1 + (iter + 1) / num_its, iter + 1, lr, losses.avg, accuracy.avg,personCorr.avg,MSE_.avg), \
                  end='', flush=True)
